function [x,f,outinfo] = LTRL2_LEC_V1(fun,const,x0,params)
% LTRL2_LEC_V1 - Limited-Memory Trust-Region Shape-Changing Norm for
...
b2c         = zeros(mm,1); % Copy of b2. Used in the computing the performance metric.
        ...
        while (ratio<=tau0)
            ...
            while abs(phi) > epsroot && itroot < maxitroot
                ...
                if itroot > 0
                    ...
                    st                  = -del.*(V(:,maskV)*p(1:numsy2) + gradp);
