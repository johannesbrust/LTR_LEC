function [x,f,outinfo] = LTRSC_LEC_V1(fun,const,x0,params)
% LTRSC_LEC_V1 - Limited-Memory Trust-Region Shape-Changing Norm for
...
J.B., 07/17/18
LTRSC_LEC_V1 implements the trust-region acceptance criterion:
...
while (mflag==1)    
    ...
    p(1:numsy2)         = (Uc(1:numsy2,rankA+1)- Uc(1:numsy2,1:rankA)*b2(1:rankA))./nV(maskV);
    
    st                  = -alpha.*(V(:,maskV)*p(1:numsy2) + gradp);
    ...
    if nst<= (trrad+trradb)  % quasi-Newton step is inside TR
        ...
        elseif ared<0
            ...
            pred=-0.5*alpha*(ng^2+p(1:numsy2)'*(Vg(1:numsy2).*nV(maskV)) - Ag(1:rankA)'*b2(1:rankA));
            ratio=ared/pred;
    ...
    if ~Nflag
        ...
        ngperp          = sqrt(max(0,ngp2-ngpar*ngpar));    
...
%             end
            ...
            elseif ared<0
                ...
                pred    = (gpar(1:rankV)'*vpar(1:rankV) + alpha*(-1+0.5*gp*alpha)*ngperp^2 + ... % delta
    ...
    % Computing \| Proj(g) \|^2
    ngp2            = ng^2 - Ag(1:rankA)'*b2(1:rankA);
    ngp             = sqrt(abs(ngp2));
